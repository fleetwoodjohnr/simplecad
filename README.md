...
| **Direct manipulation** | Select a face and **drag it** to push/pull, with a live translucent preview and the **resulting size** shown at the cursor as you go — not just how far you have moved. **Round faces drag too**: pull the side of a shaft or a tube toward its axis and it gets thinner, with the resulting diameter at the cursor. The tool is a ring between the old and the new radius, never a solid cylinder, so a tube keeps its bore. Pushing inward turns the body translucent so the material being removed is actually visible, and the drag snaps to 0.25 mm (Shift for free, Ctrl for 0.05 mm) so landing on a round number is the default. **Fillet and Chamfer put a handle on the edge** and you pull the corner to size. **Letting go commits**, exactly as letting go of a dragged face does. The larger, screen-stable **transform gizmo** attaches to every selected body, shows the original positions as a ghost and reports live ΔX/ΔY/ΔZ beside the cursor, so a multi-object move has both direction and depth cues |
...
| **Primitives** | Box, Cylinder, Sphere, Cone, Torus, Tube, Wedge — created and sized without a sketch. **Triangle, Pentagon, Hexagon and Octagon** prisms as their own tiles (even-sided sized across flats, odd-sided by their corner circle), plus a Polygon prism for any other side count. Decorative **Star, Heart, Cross, Crescent and Lightning** silhouettes expose useful proportions and extrusion height. **Vent**: a hex grille for a fan, as a plate of its own or cut straight through a wall you already have |
| **Modelling** | Press/Pull, Move, Rotate, Scale, Fillet, Chamfer, **Hollow** (shell to a wall thickness — pick the face to open and a box becomes a bucket), Hole (simple, counterbore, countersink, threaded). Select a flat face and **Add text** to raise a label or engrave it inward, with live preview, Sans/Serif/Monospace families, exact depth, face-local offsets and rotation |
...
| **Align / Stack / Mate** | Stack, Center and Concentric with Flip and normal offset. **Place on Face** adds face-local X/Y coordinates from Left/Center/Right and Bottom/Center/Top anchors, with a live preview and persistent relationship. **Arrange** keeps three or more separate bodies in line with either equal clear gaps or an entered gap; an optional target face seats and orients their support faces |
| **Threads** | A **printable coarse series** plus ISO Metric, UNC, UNF, BSP and NPT (100 sizes), with the printed series offered first because SimpleCAD makes parts for a printer. The tooth is a **45-degree trapezoid with flats at crest and root**, so nothing on the thread overhangs past 45° and it prints axis-up without supports — an ISO 60° V tooth overhangs by 63° and needs support inside the thread, which is what stops a printed pair ever turning. Both ends taper away over the last turn so the parts start by hand. Thread and every matching-part mode show the **actual helical kernel result live** and only enable Create after it succeeds — in the geometry process, or in-process when that cannot start, so the answer always arrives; an unchanged cosmetic cylinder is never accepted as a thread. **Any round face can be threaded**: every standard size is offered on it, each labelled with what choosing it would do (*opens the hole 4.00 mm*), and a face that is not the size the thread wants is opened out — or built up — on request, which is what a tap drill is for. A hole too small to take the thread at all says so with both diameters rather than failing. **Create Matching Part** locks size, form, hand and clearance to the source and enforces opposite internal/external polarity, and it **opens on whatever you selected**: a flat face on the other part drills and threads it, a bore or a shaft threads that, a whole tube finds its one bore, and with nothing selected you get a free-standing bolt or nut placed visibly beside the model |
...
| | |
|---|---|
| Orbit | Shift + middle-drag, or Alt + left-drag |
| Pan | Middle-drag; Ctrl+Shift + two-finger scroll on a trackpad |
| Zoom | Wheel |
...
| Raise or engrave text | Select a flat face, then Add Text |
...
.venv/bin/python scripts/check_text.py       # raise and engrave labels through the UI
