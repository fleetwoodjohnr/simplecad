...
from ...kernel.arrange import ArrangeFeature, arrange_shapes
...
class ArrangePanel(_SelectionTool):
    ...
    def build(self) -> None:
        ...
        self.axis = QComboBox()
        self.axis.addItem("Local X" if target is not None else "World X", "x")
        self.axis.addItem("Local Y" if target is not None else "World Y", "y")
        if target is None:
    ...
    def _answer(self):
        ...
        items = []
        for name in names:
            body = self.window_.document.body(name)
            if body is not None and body.shape is not None:
                items.append((name, body.shape))
        return arrange_shapes(
            items,
            mode=str(self.mode.currentData()),
            axis=str(self.axis.currentData()),
            gap=self.value("gap", 10.0),
            target_face=target.shape if target is not None else None,
            orient=self.orient.isChecked() if target is not None else False,
            normal_offset=self.value("normal_offset", 0.0),
    ...
    def preview(self) -> None:
        ...
        try:
            answer = self._answer()
        except Exception as exc:  # noqa: BLE001 - panel stays open for correction
            from ...core.errors import translate

            self.confirm.setEnabled(False)
            self.warn(str(translate(exc, "arrange")))
            return
        viewport = self.window_.stage.viewport
        viewport.show_ghost(
            compound(answer.shapes.values()),
            self.window_.palette_.accent,
            transparency=0.12,
        )
        self._preview_names = list(answer.shapes)
        for name in self._preview_names:
            viewport.set_transparency(self.window_._presentations.get(name), 0.82)
        note = f"Resulting clear gap: {answer.gap:.3f} mm"
        if answer.warnings:
            note += " · " + " · ".join(answer.warnings)
        self.warn(note)
    ...
    def commit(self) -> None:
        ...
        if target is not None:
            from ...kernel.align import _dot, planar_frame

            frame = planar_frame(target.shape)
            inputs.update({
